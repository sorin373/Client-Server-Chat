...
namespace net::interface
{
    ...
    void user::buildIndexHTML(void)
    {
        ...
        {
            strcpy(fileName, uf[i].file_name());
    ...
    void user::SQLfetchUserTable(void)
    {
        ...
        sql::ResultSet *res = m_mysql_handle->send_query("SELECT * FROM user");
    ...
    void user::SQLfetchFileTable(void)
    {
        ...
        sql::ResultSet *res = m_mysql_handle->send_query("SELECT * FROM file WHERE user_id=" + std::to_string(getSessionID()));
        ...
        while (res->next())
        {
            ...
            user_files t_uf(fileName, userID, fileID, fileSize, t_date);
    ...
    int user::addToFileTable(const char *fileName, const double fileSize)
    {
        ...
        for (const user_files &t_uf : uf)
            if (strcmp(t_uf.file_name(), fileName) == 0)
            {
                found = true;
                break;
            }
        ...
        sql::ResultSet *res = m_mysql_handle->send_query("SELECT file_id FROM file");
        ...
        res = m_mysql_handle->send_query("SELECT sysdate() As date FROM dual");
    ...
    int user::routeManager(void *buffer, char *route, int acceptedSocketFD, ssize_t bytesReceived)
    {
        ...
        if (strstr(route, "/userlogin"))
            if (loginRoute(charBuffer, acceptedSocketFD) == EXIT_FAILURE)
                return EXIT_FAILURE;
        ...
        if (strstr(route, "/addFile"))
        {
            ...
            if (addFilesRoute(charBuffer, byteBuffer, acceptedSocketFD, bytesReceived) == EXIT_FAILURE)
                return EXIT_FAILURE;
        }

        if (strstr(route, "/change_password"))
            if (changePasswordRoute(charBuffer, acceptedSocketFD) == EXIT_FAILURE)
                return EXIT_FAILURE;

        if (strstr(route, "/create_account"))
            if (createAccountRoute(charBuffer, acceptedSocketFD) == EXIT_FAILURE)
                return EXIT_FAILURE;

        if (strstr(route, "/delete_file"))
            if (deleteFileRoute(charBuffer, acceptedSocketFD) == EXIT_FAILURE)
                return EXIT_FAILURE;
    ...
    int user::deleteFileRoute(char *buffer, int acceptedSocketFileDescriptor)
    {
        ...
        prepStmt = m_mysql_handle->connection()->prepareStatement(deleteQuery);
